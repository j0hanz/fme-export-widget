...
export const inputToFmeDateTime = (v: string, original?: string): string => {
  ...
  if (!date || !time) {
    logWarn("inputToFmeDateTime: Invalid ISO format", v);
    return "";
  }
  ...
  if (!y || y.length !== 4 || !/^\d{4}$/.test(y)) {
    logWarn("inputToFmeDateTime: Invalid year", y);
    return "";
  }
  ...
  if (!m2 || !d2 || !hh2 || !mi2) {
    logWarn("inputToFmeDateTime: Invalid date/time components", {
      m,
      d,
      hh,
      mi,
  ...
  if (ss2 === null) {
    logWarn("inputToFmeDateTime: Invalid seconds", ssRaw);
    return "";
  }
