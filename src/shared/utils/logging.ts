...
export const conditionalTable = (
  ...
  if (isLoggingEnabled) {
    console.table(data, columns as string[]);
  }
};
...
export const logDebug = conditionalLog;
export const logWarn = conditionalWarn;
...
const getDebugObject = (): FmeDebugObject | null => {
  ...
  return (window as { __FME_DEBUG__?: FmeDebugObject }).__FME_DEBUG__ ?? null;
};
...
const safeAssignDebugObject = (
  ...
  try {
    ...
    try {
      (target as { __FME_DEBUG__?: FmeDebugObject }).__FME_DEBUG__ = debugObj;
    } catch {
...
const safeGetDebugObject = (target: Window): FmeDebugObject | null => {
  try {
    return (target as { __FME_DEBUG__?: FmeDebugObject }).__FME_DEBUG__ ?? null;
  } catch {
    return null;
  }
};
...
const createDebugObject = (context: {
  ...
  utils: {
    ...
    formatArea: (area: number, spatialReference?: __esri.SpatialReference) =>
      formatArea(
        area,
        createMockIntlModules() as unknown as Parameters<typeof formatArea>[1],
        spatialReference
      ),
